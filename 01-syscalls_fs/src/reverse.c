...
void reverse_file_data(int src_fd, int dst_fd)
{
    ...
    if (file_size == -1) {
        print_error("[ERROR] Error getting file size");
        close_file(src_fd);
        close_file(dst_fd);
        exit(EXIT_FAILURE);
    }
    ...
    for (offset = file_size; offset > 0; offset -= BUFFER_SIZE) {
        ...
        if (lseek(src_fd, offset - to_read, SEEK_SET) == -1) {
            print_error("[ERROR] Error seeking in source file");
            close_file(src_fd);
            close_file(dst_fd);
            exit(EXIT_FAILURE);
        }
...
void reverse(const char *src, const char *dst) {
    ...
    print("[INFO] Reversing data from %s to %s...\n", src, dst);
